...
figure(3)
Rectifier_efficiency_array = [0.983:0.001:0.991];
...
% total steps
number_of_sample_points = 20; %for example.
...
filter_efficiency_array = [filter_efficiency_low: filter_efficiency_step: filter_efficiency_high];

efficiency_array = chopper_efficiency_array .* inverter_transformer_efficiency_array .* rectifier_efficiency_array .* filter_efficiency_array .* filter_efficiency_array;
...
mass_mesh = Calculate_Chopper_Stage_Mass(F1, Vin, Vout, Pout, Available_Modules, Required_Modules, chopper_efficiency_array) ...
    + Calculate_Inverter_Transformer_Stage_Mass(F1, Vin, Vout, Pout, Available_Modules, Required_Modules, inverter_transformer_efficiency_array) ...
    + Calculate_Rectifier_Stage_Mass(F1, Vin, Vout, Pout, Available_Modules, Required_Modules, rectifier_efficiency_array) ...
    + Calculate_Filter_Stage_Mass(F1, 120, Pout, Available_Modules, Required_Modules, filter_efficiency, FS_ripple) ...
    + Calculate_Filter_Stage_Mass(F1, 5000, Pout, Available_Modules, Required_Modules, filter_efficiency, FS_ripple);

specific_power_mesh = Pout./mass_mesh;


surf(power_mesh, efficiency_mesh, specific_power_mesh)
xlabel('Power [kW]')
ylabel('Efficiency')
zlabel('Specific Power [kW/kg]')
title('DDCU Stage')
