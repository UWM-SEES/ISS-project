...
%Oxygen_production_total_power =Habitat_power;
%% Fission Surface Power

Power_per_reactor = 10e3;
Number_of_reactors = 4;
Total_fisson_power = Power_per_reactor*Number_of_reactors;

reactor_mass = 1545;
Total_fisson_mass = reactor_mass*Number_of_reactors; 
...
E_Load = Oxygen_production_total_power*t_d; %Wh
...
A_a = 1;
P_a = PV_Array_constants.*A_a.*sin(alpha + theta);
E_Load = Oxygen_production_total_power*t_d;
%E_ox = 25.8e3*t_d;
P_a_pos = zeros(1, length(P_a));
%k =1
...
Load_total_power_plot = zeros(1, length(P_a));
Load_net_power_plot = zeros(1, length(P_a));
Ox_prod_power_plot = zeros(1, length(P_a));
Habitat_power_plot = zeros(1, length(P_a));
Total_fission_power_plot = zeros(1, length(P_a));
...
% for i = 1:length(t_i)
%     if t_i(i) < 143
%          Ox_production_power_plot(1,i) = 2*Oxygen_production_total_power+Habitat_power;
%     elseif t_i(i) >= 143 && t_i(i) < 565
%          Ox_production_power_plot(1,i) = 0.2*Oxygen_production_total_power + 0.2*Habitat_power;
%     else
%         Ox_production_power_plot(1,i) = 2*Oxygen_production_total_power + Habitat_power; 
%     end
%     E_ox = E_ox + Ox_production_power_plot(1,i)*delta_t;
% end

for i = 1:length(t_i)
    if t_i(i) < 143
        Ox_prod_power_plot(1, i) = Oxygen_production_total_power;
        Habitat_power_plot(1, i) = Habitat_power;
        Total_fission_power_plot(1, i) = Total_fisson_power;
    ...
    Load_total_power_plot(1,i) = Oxygen_production_total_power + Habitat_power;
    Load_net_power_plot(1,i) = Oxygen_production_total_power + Habitat_power - Total_fisson_power; 
    E_Load = E_Load + Load_net_power_plot(1,i)*delta_t;
end



%Ox_production_power_plot(1,:) = Oxygen_production_total_power + Habitat_power;



%% keep running loop while PV farm energy <= Energy needed for O_2 production
while Final_PV_Farm_energy <= E_Load
...
ylabel('Power (kW)')
xlim([0 t_d])
stairs(t_i, Load_net_power_plot/1000)
...
while ESS_final_energy < 0
    ...
    ESS_power_time = zeros(1, length(P_a_pos));

    Load_profile = Load_net_power_plot;


     if(Load_profile(i) > P_a_pos(i))
           ESS_power_time(i) = Load_profile(i) - P_a_pos(i);
           ESS_energy_time(i) = ESS_power_time(i)*delta_t;
     end
    for i =2:1:length(P_a)
        ...
        if(Load_profile(i) > P_a_pos(i))
           ...
           %ESS_power_time(i) = Load_profile(i) - P_a_pos(i);
           ESS_power_time(i) =  P_a_pos(i) - Load_profile(i) ;

           %new energy value
           ESS_energy_time(i) = ESS_energy_time(i-1) +ESS_power_time(i)*delta_t/ESS_EFFICIENCY_DISCHARGE;
        else
        %ESS will charge

            ESS_power_time(i) = P_a_pos(i) - Load_profile(i);
            ESS_energy_time(i) = ESS_energy_time(i-1) + ESS_power_time(i)*delta_t*ESS_EFFICIENCY_CHARGE;
        end  
    end
...
ESS_charge_max = max(abs(ESS_power_time));
...
%% Add positive and negative for BESS plots
%Take ESS power data and seperates positive and negative values

%length of variable
ESS_power_time_length = length(ESS_power_time);

%initalize variables to avoid change array size in loop, which slows down
%matlab
ESS_power_positive_time = zeros(1, ESS_power_time_length);
ESS_power_negative_time = zeros(1, ESS_power_time_length);

for ii = 1:ESS_power_time_length
   if(ESS_power_time(ii) >= 0)
       ESS_power_positive_time(ii) =0;
       ESS_power_negative_time(ii) = ESS_power_time(ii);%to make negative power postivie
   else
      ESS_power_positive_time(ii) =-ESS_power_time(ii);
      ESS_power_negative_time(ii) = 0; 
   end
end
%% Total power and total loads
total_power_generation_time = P_a_pos+Total_fission_power_plot+ESS_power_positive_time;
total_load_time = ESS_power_negative_time+Ox_prod_power_plot+Habitat_power_plot;
...
ylabel('Power (kW)')
xlim([0 t_d])
stairs(t_i, Load_total_power_plot/1000, 'linewidth', 1.25)
%stairs(t_i, Load_net_power_plot/1000, 'linewidth', 1.25)
stairs(t_i, Ox_prod_power_plot/1000, 'linewidth', 1.25)
stairs(t_i, Habitat_power_plot/1000, 'linewidth', 1.25)
stairs(t_i, Total_fission_power_plot/1000, 'linewidth', 1.25)
stairs(t_i, ESS_power_time/1000, 'linewidth', 1.25)

legend('PV', 'Total Load', 'ISRU Ox','Habitat', 'Fission Power', 'BESS Power')
...
xlim([0 t_d])
stairs(t_i, Total_fission_power_plot/1000, 'linewidth', 1.25)
stairs(t_i, ESS_power_positive_time/1000, 'linewidth', 1.25)
stairs(t_i, total_power_generation_time/1000, 'linewidth', 1.25)

legend('PV', 'Fission Power', 'BESS Discharge', 'Total Power')
title('Total and Individual Power Generation Profiles')

fig2.CurrentAxes.FontName = 'Times New Roman';
fig2.CurrentAxes.FontSize = 12;
fig2.CurrentAxes.FontWeight = 'Bold';
fig2.CurrentAxes.GridAlpha = 0.5;
fig2.CurrentAxes.LineWidth = 1;
...
stairs(t_i, ESS_power_negative_time/1000, 'linewidth', 1.25)
stairs(t_i, total_load_time/1000, 'linewidth', 1.25)


legend('ISRU Ox','Habitat', 'BESS Charging', 'Total Load Power')
title('Total and Individual Loads')
...
%% Display
fprintf("Mass of Lunar Base DERs: %4.3f kg \n", Total_fisson_mass + ISRU.PV.Mass_PV_panels + ISRU.PV.Mass_PV_Structure + ISRU.BESS.Battery_mass + ISRU.BESS.Mass_total_thermal_system)
...
fprintf("\t Total Mass of Fission Surface Power: : %4.3f kg \n", Total_fisson_mass)
