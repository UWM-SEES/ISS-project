...
Load_power_plot = zeros(1, length(P_a));
...
for i = 1:length(t_i)
    ...
    Load_power_plot(1,i) = Oxygen_production_total_power + Habitat_power - Total_fisson_power; 
    E_Load = E_Load + Load_power_plot(1,i)*delta_t;
end
...
ylabel('Power (kW)')
xlim([0 t_d])
stairs(t_i, Load_power_plot/1000)
...
ylabel('Power (kW)')
xlim([0 t_d])
stairs(t_i, Load_power_plot/1000, 'linewidth', 1.25)
...
legend('PV', 'Net Load', 'ISRU Ox','Habitat', 'Fission Power')
